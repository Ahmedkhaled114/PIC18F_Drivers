 ...
 * Author: Ahmed
 *
 * Created on March 30, 2021, 6:41 AM
 */
...
static void lcd_8bit_intialize(const char_lcd_t *_lcd){
    ...
    gpio_pin_write_value(_lcd->lcd_en_port, _lcd->lcd_en_pin, LOW);
    gpio_pin_direction_initialize(_lcd->lcd_rs_port, _lcd->lcd_rs_pin, DIRECTION_OUTPUT);
    gpio_pin_write_value(_lcd->lcd_rs_port, _lcd->lcd_rs_pin, LOW);
    /*
    __delay_ms(40);
...
#if CHAR_LCD_MODE==8
    gpio_pin_write_value(_lcd->lcd_en_port, _lcd->lcd_en_pin, HIGH);
    __delay_us(200);
    gpio_pin_write_value(_lcd->lcd_en_port, _lcd->lcd_en_pin, LOW);
...
ret_status lcd_send_char_data(const char_lcd_t *_lcd, uint8_t _data){
    ...
    if(NULL == _lcd){
        ...
        lcd_send_enable(_lcd);
        #elif CHAR_LCD_MODE==8
        gpio_pin_write_value(_lcd->lcd_rs_port, _lcd->lcd_rs_pin, HIGH);
